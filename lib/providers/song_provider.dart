...
class SongProvider extends ChangeNotifier {
  ...
  Future<void> _loadLPMISongs() async {
    final String jsonString = await rootBundle.loadString('assets/data/lpmi_songs.json');
  ...
  Future<void> _loadSRDSongs() async {
    final String jsonString = await rootBundle.loadString('assets/data/srd_songs.json');
    final List<dynamic> jsonData = json.decode(jsonString);
  ...
  Future<void> _loadFavorites() async {
    ...
    for (String id in favoriteIds) {
      ...
      final lpmiMatch = _lpmiSongs.where((song) => 
        "${song.songNumber}_${AppStrings.lpmiCollection}" == id).toList();
      if (lpmiMatch.isNotEmpty) {
        _favoriteSongs.add(lpmiMatch.first);
      }
      
      // Look in SRD collection
      final srdMatch = _srdSongs.where((song) => 
        "${song.songNumber}_${AppStrings.srdCollection}" == id).toList();
  ...
  void _applySortingAndFiltering() {
    ...
    List<UnifiedSong> collectionSongs = 
        _currentCollection == AppStrings.lpmiCollection ? _lpmiSongs : _srdSongs;
  ...
  void _applySorting() {
    if (_sortOption == AppStrings.sortByNumber) {
      ...
      _filteredSongs.sort((a, b) {
        ...
        int? numA = int.tryParse(a.songNumber.replaceAll(RegExp(r'[^0-9]'), ''));
        int? numB = int.tryParse(b.songNumber.replaceAll(RegExp(r'[^0-9]'), ''));
  ...
  Future<void> toggleFavorite(UnifiedSong song) async {
    ...
    if (isFavorite) {
      ...
      _favoriteSongs.removeWhere((s) => 
        ...
        ((_currentCollection == AppStrings.lpmiCollection && _lpmiSongs.contains(s)) || 
         (_currentCollection == AppStrings.srdCollection && _srdSongs.contains(s)))
      );
    } else {
      // Add to favorites
  ...
  Future<void> playOrPauseSong(UnifiedSong song) async {
    if (_playingSongNumber == song.songNumber && _isPlaying) {
      ...
      await _audioPlayerService.pause();  // Keep await since it returns Future<void>
      _isPlaying = false;  // Set state directly
    } else if (_playingSongNumber == song.songNumber && !_isPlaying) {
      // Resume paused song
      await _audioPlayerService.resume();  // Keep await since it returns Future<void>
      _isPlaying = true;  // Set state directly
    } else {
      // Play new song
      final String songUrl = song.url ?? '';
      if (songUrl.isNotEmpty) {
        await _audioPlayerService.playOrPause(songUrl, song.songTitle);  // Keep await
  ...
  UnifiedSong? getCurrentPlayingSong() {
    ...
    List<UnifiedSong> otherSongs = 
      _currentCollection == AppStrings.lpmiCollection ? _srdSongs : _lpmiSongs;
  ...
  Future<void> stopSong() async {
    await _audioPlayerService.stop();  // Keep await since it returns Future<void>
  ...
  Future<void> seekTo(Duration position) async {
    _audioPlayerService.seek(position);  // This doesn't need await as it's not a Future
  }
  
  // Seek relative to current position
  Future<void> seekRelative(Duration offset) async {
    final newPosition = _currentPosition + offset;
    _audioPlayerService.seek(newPosition);  // This doesn't need await as it's not a Future
  }
